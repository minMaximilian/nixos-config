{colors}: {
  # Shell colors
  fish_color_normal = colors.base05;
  fish_color_command = colors.base0D;
  fish_color_keyword = colors.base0E;
  fish_color_quote = colors.base0B;
  fish_color_redirection = colors.base0C;
  fish_color_end = colors.base0F;
  fish_color_error = colors.base08;
  fish_color_param = colors.base0A;
  fish_color_comment = colors.base03;
  fish_color_selection = "white --bold --background=${colors.base02}";
  fish_color_search_match = "bryellow --background=${colors.base02}";
  fish_color_operator = colors.base0C;
  fish_color_escape = colors.base0C;
  fish_color_autosuggestion = colors.base03;
  fish_color_cancel = colors.base08;

  # Completion pager colors
  fish_pager_color_progress = colors.base03;
  fish_pager_color_prefix = "white --bold --underline";
  fish_pager_color_completion = colors.base05;
  fish_pager_color_description = colors.base03;
  fish_pager_color_selected_background = "--background=${colors.base02}";
}
